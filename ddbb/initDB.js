...
const main = async () => {
    try {
        ...
        INSERT INTO usuarios (
            email, contraseña, nombre, activo, role
        )
        VALUES(
            "kndy_1987@hotmail.com",
        ...
        for (let i = 0; i < usuarios; i++) {
            ...
            await connection.query(`
            INSERT INTO usuarios(
                 email, contraseña, nombre, activo
            )
            VALUES (
                "${email}",SHA2("${contraseña}", 512),"${nombre}", true
            );`);
        }
        ...
        for (let i = 0; i < establecimientos; i++) {
            ...
            INSERT INTO establecimientos(
                email, contraseña, nombre, direccion, activo)
                VALUES (
                    "${email}",SHA2("${contraseña}",512),"${nombre}","${direccion}",true
